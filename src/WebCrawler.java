...
public class WebCrawler extends Thread {
    ...
    final int queue_limit = 1000;
    final int pages_limit = 5000;
    ...
    public WebCrawler(int threads_number) {
    ...
    public void DownloadPage(String url) throws Exception {
        if (url != null) {
            ...
            try {
                ...
                UUID uniqe_id = java.util.UUID.randomUUID();
                BufferedWriter writer = new BufferedWriter(new FileWriter("pages/" + (uniqe_id) + ".html"));
                writer.write(doc.outerHtml());
                writer.close();
                PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("index.txt", true)));
                out.append(uniqe_id + " " + url + "\n");
                out.close();
...
//                e.printStackTrace();
    ...
    public void Crawler() throws Exception {
        ...
        pages_counter = (int) visited_table.count();
        feedCrawler();
        //Running Threads
        for (int i = 0; i < threads_number; i++) {
            Thread t = new Thread((new WebCrawler(threads_number)));
            threads.add(t);
            t.start();
        }
